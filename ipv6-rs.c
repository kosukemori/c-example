...
main (int argc, char **argv)
{
	...
	icmphdr->icmp6_type = ND_ROUTER_SOLICIT;  // 133 (RFC 4861)
	...
	*/
	...
	psdhdrlen = 16 + 16 + 4 + 3 + 1 + ICMP_HDRLEN;
	...
	*((int *) CMSG_DATA (cmsghdr)) = hoplimit;
	...
	psdhdr[34] = (ICMP_HDRLEN)  / 256;  // Upper layer packet length
	psdhdr[35] = (ICMP_HDRLEN)  % 256;  // Upper layer packet length
	...
	memcpy (psdhdr + 40, outpack, (ICMP_HDRLEN) * sizeof (uint8_t));
